...
box_auth(client_id     = "9g407svg0yqsif8bty4fc9xre37g6p5k",
         client_secret = "DBfYvIEhGJGSCmUODVVisSiSyvLyCFAV",
         interactive   = FALSE)
```
...
Philadelphia's Office of Multimodal Planning documented this pattern directly in an analysis of eight multilane arterials on the city's High Injury Network (TESC, 2024). The finding is stark:
...
<br>— OTIS Office of Multimodal Planning, TESC 2024
</div>
...
The mechanism operates through **driver attention**. Wide corridors with few visual interruptions trigger a near-automatic driving state — the same attentional mode activated on highways — in which drivers perceive minimal risk and unconsciously increase speed. At a corridor width of 50 feet (the cross-section of a typical 4-lane undivided arterial), **80% of drivers are potentially multitasking**:
...
# Monitored segments: filter centerlines to the 121 monitored seg_ids
...
The pattern is consistent and stark: **off-peak hours carry substantially higher speeding rates than peak hours across every hour of the day.** The gap is widest in the early morning (1–5 AM), where the share of speeding traffic is nearly double the midday peak. Even the off-peak midday window (11 AM–4 PM) shows noticeably elevated speeding compared to morning and evening commute windows. This establishes that the off-peak speeding phenomenon is not confined to late nights — it is a persistent feature of low-volume conditions throughout the day.
...
```{r volume-speeding, message=FALSE, warning=FALSE}
speed_data_analysis %>%
  filter(!is.na(roadway_type)) %>%
...
features <- tribble(
  ...
  # Volume & crash context
  "Volume",               "volume_total",                 "Continuous",  "Total vehicles observed during the measurement period",
  "Crash Context",        "crashes",                      "Continuous",  "Crashes on segment during this time period (2020–2024), period-matched",
  "Crash Context",        "ksi_rate",                     "Continuous",  "KSI crash rate for this segment and period, period-matched"
)
...
This aggregation reduces sparsity (late-night hours have very few measurements per segment) and focuses the model on the policy-relevant peak vs. off-peak distinction.

**Crash context — period-matched.** Crash counts and KSI rates are joined to modeling observations using both `seg_id` *and* `speed_measurement_period`, so the model sees crashes specific to each time window rather than a segment-wide total.

## 4.4 Model Architecture

We use a **Random Forest regression** with `ranger` as the engine. `mtry` (features sampled per split) and `min_n` (minimum node size) are tuned jointly over a regular grid; `trees` is fixed at 500.
...
The reproducibility seed throughout is **2718** — Euler's number × 1000. The final model object (trained on the full training set with the selected hyperparameters) is stored in Box and loaded above.

## 4.5 Model Performance

The final model was selected by minimizing RMSE across the hyperparameter tuning grid, then evaluated on the held-out 25% test set.

```{r performance-table, echo=FALSE}
performance <- tribble(
  ~Set,                            ~MAE,       ~RMSE,      ~`R²`,
  "10-fold CV (tuning grid)",      "—",        "~0.074",   "—",
...
explainer <- explain_tidymodels(
  ...
  y      = modeling_train %>% pull(all_of(target_variable)),
  label  = "Random Forest",
  verbose = FALSE
)
```
...
The permutation importance chart reveals a clear hierarchy. **`speed_measurement_period`** (time of day) is by far the dominant predictor — shuffling it causes the largest RMSE increase of any variable, reflecting the fundamental off-peak vs. peak divide documented throughout Section 2. **Road geometry variables** — particularly `traffic_lanes_width`, `curb_to_curb_width`, and `width_per_traffic_lane` — collectively represent the next tier, confirming that the physical structure of the roadway is the primary structural risk factor. **`road_classification_city`** captures the coarser typology effect (Major vs. Minor Arterial vs. Collector), which the width variables do not fully absorb. **`speed_limit`** ranks mid-tier: posted limits encode some road design intent, but they are not the binding constraint on actual operating speeds — road geometry and time of day dominate. **Intervention variables** (`bike_lane_status`, `traffic_calming`, `parking`) rank lower individually but are the most *policy-actionable* predictors: unlike road classification or curb width, they can be changed without full reconstruction. **Crash context variables** (`crashes`, `ksi_rate`) contribute meaningfully, validating that historical crash experience adds predictive signal beyond geometry alone.

### 4.6.2 Partial Dependence Plots

**Partial Dependence Plots (PDPs)** show the marginal effect of each predictor while all other predictors are held at their observed values.
...
Off-peak (night) carries the highest predicted speeding rate of all four periods, and Peak (morning) the lowest, with Peak (evening) and Off-peak (midday) falling between them in that order. The ordering — and the gap between off-peak (night) and all other periods — is visible directly in the PDP line above. This is policy-relevant: the same road, with identical geometry, produces meaningfully different predicted speeding depending purely on time of day. The fact that off-peak (midday) also exceeds both peak periods confirms that the driver mechanism is low-volume conditions rather than darkness specifically. Interventions that target road design will reduce speeding across all four periods, whereas time-of-day restrictions (e.g., speed cameras only at night) address only part of the problem.
...
p_density <- ggplot(modeling_train, aes(x = traffic_lanes_width)) +
...
The PDP shows that predicted speeding increases as traffic lane width increases — the relationship is monotonic and visible in the line above. The density plot below shows the distribution of `traffic_lanes_width` values in the training data; most segments fall in the lower-to-mid range of the width axis. The steepest portion of the curve (where additional width produces the largest incremental increase in predicted speeding) is apparent in the chart. Read in conjunction with the density plot, this means the gradient is well-supported by training data across the bulk of the distribution. Road diet scenarios that reduce lane width would, by the model's prediction, move segments leftward along this curve toward lower predicted speeding.
...
The interaction PDP shows predicted speeding by traffic lane width for each time period separately. The ordering of lines (which period is highest at any given width) and whether the lines diverge or converge as width increases are both readable from the chart above. The key observable pattern: the period lines are not parallel — the gap between them changes as lane width changes, indicating that the effect of lane width on predicted speeding is not uniform across time periods. Specifically, the chart shows that the off-peak (night) line sits above the other period lines, and that this separation is more pronounced at wider lane values. This means the predicted benefit of reducing lane width is largest for the highest-risk time window.

</div><div class="pdp-section">

#### Curb-to-Curb Width

```{r pdp-curb-width, message=FALSE, warning=FALSE, fig.cap="Total roadway cross-section independently predicts higher speeding, even after controlling for lane count and lane width."}
pdp_curbwidth <- model_profile(explainer, type = "partial",
                                variables = "curb_to_curb_width", N = NULL)

p_cw <- as_tibble(pdp_curbwidth$agr_profiles) %>%
  ggplot(aes(x = `_x_`, y = `_yhat_`, group = `_label_`)) +
  geom_line(linewidth = 1.3, color = palette_blue) +
  scale_y_continuous(labels = label_percent(), limits = c(0, NA),
                     expand = expansion(mult = c(0, 0.2))) +
  labs(title = "Predicted speeding by curb-to-curb width",
       subtitle = "Wider total cross-sections predict higher speeding independent of lane count",
       y = "Predicted speeding rate", x = "Curb-to-curb width (ft)")

p_cw_density <- ggplot(modeling_train, aes(x = curb_to_curb_width)) +
  geom_density(fill = palette_blue, alpha = 0.25, color = palette_blue) +
  scale_y_reverse() +
  theme_minimal() +
  theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
...
p_cw / p_cw_density + plot_layout(heights = c(4, 1))
```

The curb-to-curb PDP shows an upward-sloping relationship between total roadway width and predicted speeding, independent of `traffic_lanes_width`. The fact that both width variables retain predictive power in the model (as confirmed by variable importance) means the total cross-section carries information about speeding beyond what lane width alone captures. In practical terms, this suggests that interventions which reduce total curb-to-curb width — even without changing the number of travel lanes — would, according to the model, also reduce predicted speeding. The density plot shows the distribution of curb-to-curb values in the training data; the PDP is well-supported across the middle of that range.

</div><div class="pdp-section">

#### Bike Lane Status

```{r pdp-bike, message=FALSE, warning=FALSE, fig.cap="Separated bike lanes show the strongest speed-reducing association. Sharrows and painted lanes show smaller but consistent effects."}
pdp_bike <- model_profile(explainer, type = "partial",
                           variables = "bike_lane_status", N = NULL)

p_bike <- as_tibble(pdp_bike$agr_profiles) %>%
  mutate(`_x_` = fct_relevel(`_x_`, "None", "Sharrow", "Painted", "Separated")) %>%
  ggplot(aes(x = `_x_`, y = `_yhat_`, group = `_label_`)) +
  geom_line(linewidth = 1.3, color = palette_blue) +
  scale_y_continuous(labels = label_percent(), limits = c(0, NA),
                     expand = expansion(mult = c(0, 0.2))) +
  labs(title = "Predicted speeding by bike lane status",
       y = "Predicted speeding rate", x = "Bike lane type")

p_bike_bar <- ggplot(modeling_train, aes(x = fct_relevel(bike_lane_status,
                        "None","Sharrow","Painted","Separated"))) +
  geom_bar(fill = palette_blue, alpha = 0.25, color = palette_blue) +
  scale_y_reverse() +
  theme_minimal() +
  theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
        axis.ticks.y = element_blank(), panel.grid = element_blank()) +
...
p_bike / p_bike_bar + plot_layout(heights = c(4, 1))
```

The PDP shows a clear monotonic relationship: **None > Sharrow > Painted > Separated**, with predicted speeding declining at each step of protection. The largest single drop occurs between Painted and Separated — consistent with the Chicago Dearborn Street evidence reviewed in Section 2.4, where physical protection structures created a geometric narrowing effect that reduced motor vehicle operating speeds even in the absence of cyclists. Sharrows and painted lanes show smaller but meaningful effects, likely through a combination of narrowed perceived travel lane width and driver awareness of shared space. The bar chart below shows that most monitoring network segments currently have no bike infrastructure — indicating that upgrading even to painted lanes across this network would shift a large portion of the distribution toward lower predicted speeding rates.
...
Traffic calming devices (speed humps, cushions, raised crossings) show a clear, discrete protective association — the step-down from no calming to calming is among the cleaner effects in the model. Unlike the continuous width variables, `traffic_calming` is a binary indicator, so the PDP displays a simple comparison between the two states. The magnitude of the protective effect is meaningful: segments with traffic calming have substantially lower predicted speeding rates, holding all other predictors constant. This effect is also borne out in the error analysis (Section 4.7): calmed segments have an RMSE of only 0.025 — the lowest of any subgroup — because the physical constraints leave little variance in actual behavior, making them both safer and more predictable. From a planning perspective, traffic calming represents one of the highest-value, lowest-cost interventions available: it is cheaper than lane removal, can be deployed rapidly, and the model confirms its effectiveness even on segments where other risk factors (wide lanes, no sidewalks) are present.
...
<div class="stat-row">
  ...
  <div class="stat-box">
    ...
    <span class="stat-label">RMSE on calmed segments<br><em>lowest-error subgroup</em></span>
  </div>
  <div class="stat-box">
...
Three diagnostic patterns emerge from the subgroup error analysis:

**High-error subgroups (Major Arterials, 5+ lanes, off-peak night)** share a common feature: they are both the most dangerous and the least represented in the monitoring network. Major Arterials and very wide roads are over-represented in KSI crashes but under-represented in training data, leading the model to rely on interpolation from similar segments. This is where prediction uncertainty is highest for the scenario simulations in Section 6, and where expanding the monitoring network would yield the greatest modeling benefit.

**Off-peak (night) is the hardest time period to predict** (RMSE = 0.055), even though the model already identifies it as the highest-risk window. This reflects genuine behavioral heterogeneity at night: late-night drivers include both commuters returning home and a smaller share of high-risk drivers with elevated impairment rates — a mix the model cannot fully disentangle from road characteristics alone.

**Traffic-calmed segments are the easiest to predict** (RMSE = 0.025 — less than half the global RMSE of 0.074). Physical calming devices sharply constrain the achievable operating speed, leaving little residual variance for the model to miss. This reinforces the policy case: traffic calming not only reduces speeding, it makes future speeding more predictable — a valuable property for planning.
...
<div class="callout callout-finding">
**Finding 4 — Lane width and total roadway cross-section are the dominant structural predictors.**
Both traffic lane width and curb-to-curb width have strong, independent associations with higher predicted speeding rates. The steepest risk gradient occurs in the 20–40 ft traffic lane width range — the profile of a typical 2-to-4-lane arterial. Per-lane width matters independently: wider individual lanes encourage higher operating speeds even holding total lanes constant.
</div>
...
**Data coverage.** Speed measurement data covers only 121 road segments, concentrated on higher-classification streets. Model predictions for local residential streets and unmeasured segments should be interpreted with caution. Error analysis confirms that Major Arterials and roads with 5+ lanes carry the highest prediction RMSE.

**Temporal imbalance.** 2022 contributes ~46% of all measurement records. Year-specific road conditions, construction, or enforcement patterns in 2022 may influence the model in ways that are difficult to disentangle.

**Predictive, not causal.** The model is predictive, not causal. Partial dependence plots reveal marginal associations, not controlled experimental effects. Scenario simulation outputs should be treated as indicative planning estimates, not precise causal forecasts.

**Monitoring site selection bias.** Speed monitoring sites may be deployed preferentially on known high-risk corridors. If so, the model may overestimate speeding risk on unmonitored streets.

**Period-level aggregation.** The model operates at the coarse period level (4 periods per day). Within-period variation, weather, special events, and hour-by-hour fluctuations are not captured.

**Width measurement uncertainty.** Despite hand-verification of all 121 monitored segments, width measurements on streets with unusual cross-sections carry some imprecision, particularly where curb placement is irregular.
...
The Random Forest model explains **92% of the variance** in period-level speeding rates (R² = 0.920, RMSE = 0.074 on the held-out test set). This level of accuracy confirms the project's central premise: **speeding is structurally determined, not random.** Time of day, road geometry (especially lane width and total cross-section), road classification, and intervention features collectively predict nearly all of the variation in observed speeding rates. The implication is that road design choices made decades ago are still producing measurable safety outcomes today — and that changing those choices will produce measurable improvements.

The partial dependence analysis identifies several actionable patterns. Traffic lane width and curb-to-curb width both independently predict higher speeding, with the width gradient well-supported across the range of Philadelphia's arterial network. Separated bike lanes, traffic calming devices, and sidewalk presence all show negative associations with predicted speeding — each representing an infrastructure class that can be deployed without full roadway reconstruction. The period-of-day effect is the strongest single predictor in the model, but it is not actionable on its own: planners cannot change what time it is. What they can change is the road.

### Recommendations

**Road redesign prioritization.** Wide (>50 ft curb-to-curb), multi-lane (≥4 lanes), undivided arterials with high predicted nighttime speeding rates and documented KSI crash histories are the strongest candidates for road diet feasibility studies. The model's scenario outputs (Section 6) identify which specific segments show the largest predicted improvement from lane reduction.

**Traffic calming deployment.** The model shows the strongest error reduction on calmed segments (RMSE = 0.025 vs. 0.052 for uncalmed) — physical calming is not only effective, it makes speeding behavior predictable. Segments with wide lanes, no existing calming, and high predicted off-peak speeding rates should be prioritized for speed cushion or hump installation.

**Bike infrastructure as a safety investment.** The PDP for bike lane status shows a monotonic decrease in predicted speeding from None through Sharrow, Painted, and Separated. Corridors in the monitoring network currently without bike infrastructure represent opportunities where even a painted lane would shift the model's prediction downward — a safety co-benefit of network expansion.

**Enforcement targeting.** Model outputs can guide speed camera placement and targeted patrol scheduling at the highest-risk late-night (10 PM–5 AM) segments, particularly on Major Arterials where prediction uncertainty is highest (RMSE = 0.071) and enforcement can supplement structural intervention.

**Monitoring network expansion.** The model's highest prediction errors occur on Major Arterials and roads with 5+ lanes — the road types with the worst KSI records and the fewest monitoring observations. Expanding pneumatic tube deployments to a more representative sample of Philadelphia arterials would simultaneously improve model accuracy and provide direct before-after measurement for completed interventions.

**Before-after evaluation.** As OTIS implements road diets, traffic calming, and bike infrastructure on High Injury Network corridors, the speed monitoring network provides a natural evaluation framework. Longitudinal comparison of modeled vs. observed speeding rates at intervention sites would validate the model's counterfactual predictions and build the evidence base for scaling up the most effective treatments.
